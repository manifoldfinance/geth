...
func TestDelta(t *testing.T) {
	...
	if err := statedb.TrieDB().Commit(root, false, func(common.Hash) {}); err != nil {
		t.Fatalf(err.Error())
	}
	destructs, accounts, storage, err := DiffTries(diskdb, root, emptyRoot)
	if l := len(destructs); l > 0 {
		t.Errorf("Expected no destructs, got %v", l)
	}
	if l := len(accounts); l != 1 {
		t.Errorf("Expected 1 account, got %v", l)
	}
