...
func (cep *mockChainEventProvider) GetFullChainEvent(ce core.ChainEvent) (*ChainEvent, error) {
	if fce, ok := cep.kv[ce.Block.Hash()]; ok {
		return fce, nil
	}
...
func TestGetProducerMessages(t *testing.T) {
	...
	if MsgType(messages[0].key[0]) != BlockMsg {
		t.Errorf("Message 0 should be block Msg, got %v", messages[0].key[0])
	}
	if MsgType(messages[1].key[0]) != TdMsg {
		t.Errorf("Message 1 should be block Msg, got %v", messages[1].key[0])
	}
	if MsgType(messages[2].key[0]) != ReceiptMsg {
		t.Errorf("Message 2 should be log Msg, got %v", messages[2].key[0])
	}
	if MsgType(messages[3].key[0]) != LogMsg {
		t.Errorf("Message 3 should be log Msg, got %v", messages[3].key[0])
	}
}
...
func TestChainEventTracker(t *testing.T) {
	...
	for i, msg := range messages {
		...
		if err != nil {
			t.Errorf("Error handling message: %v", err)
		}
		if chainEvents != nil && i < 3 {
			t.Errorf("Got chainEvents before anticipated: %v", chainEvents)
		}
	}
	if chainEvents == nil {
		t.Fatalf("Expected chainEvents object: %v", consumer.cet)
	}
	if reverted := len(chainEvents.Reverted); reverted != 0 {
		t.Errorf("Expected 0 reverts, got %v", reverted)
	}
	if newCount := len(chainEvents.New); newCount != 1 {
		t.Errorf("Expected 1 new, got %v", newCount)
	}
	if offset, ok := chainEvents.Partitions[0]; !ok || offset != 3 {
		t.Errorf("Expected offset to be 3")
	}
	blockhash := chainEvents.New[0].Block.Hash()
	if !consumer.cet.finished[blockhash] {
		t.Errorf("Block should be marked finished in CET")
	}
	if _, ok := consumer.cet.logCounter[blockhash]; ok {
		t.Errorf("logCounter should be gone for blockhash")
	}
	if _, ok := consumer.cet.receiptCounter[blockhash]; ok {
		t.Errorf("receiptCounter should be gone for blockhash")
	}
}
func handleReorderedMessages(t *testing.T, order []int) {
	...
	if offset, ok := chainEvents.Partitions[0]; !ok || offset != int64(len(order)-1) {
		t.Errorf("Expected offset to be %v", len(order)-1)
	}
	blockhash := chainEvents.New[0].Block.Hash()
	if !consumer.cet.finished[blockhash] {
		t.Errorf("Block should be marked finished in CET")
...
func reorgTester(t *testing.T, messages []chainEventMessage, expectedEvents []*ChainEvents, lastEmitted common.Hash) *KafkaEventConsumer {
	...
	for i, msg := range messages {
		...
		if chainEvents != nil {
			outputs = append(outputs, chainEvents)
		}
	}
	...
	for i, chainEvents := range expectedEvents {
		if len(chainEvents.New) != len(outputs[i].New) {
			t.Fatalf("Expected events[%v]: %v New, got %v", i, len(chainEvents.New), len(outputs[i].New))
		}
		for j, chainEvent := range chainEvents.New {
			if chainEvent.Block.Hash() != outputs[i].New[j].Block.Hash() {
				t.Errorf("Got new chain events out of order o[%v][%v]", i, j)
			}
		}
		if len(chainEvents.Reverted) != len(outputs[i].Reverted) {
			t.Fatalf("Expected events[%v]: %v Reverted, got %v", i, len(chainEvents.Reverted), len(outputs[i].Reverted))
		}
		for j, chainEvent := range chainEvents.Reverted {
			if chainEvent.Block.Hash() != outputs[i].Reverted[j].Block.Hash() {
				t.Errorf("Got reverted chain events out of order o[%v][%v]", i, j)
			}
		}
	}
	return consumer
...
func shuffledTester(t *testing.T, messages []chainEventMessage, expectedEvents []*ChainEvent, lastEmitted common.Hash) *KafkaEventConsumer {
	...
	for i, msg := range messages {
		...
		if chainEvents != nil {
			...
			if len(chainEvents.Reverted) > 0 {
				t.Errorf("Got reverted blocks, expected none")
			}
		}
	}
	if len(outputs) != len(expectedEvents) {
	...
	for i, chainEvent := range expectedEvents {
		if chainEvent.Block.Hash() != outputs[i].Block.Hash() {
			t.Errorf("Got new chain events out of order o[%v]", i)
		}
	}
	return consumer
}
...
func TestReorg(t *testing.T) {
	...
	if b.Block.ParentHash() != a.Block.Hash() {
		t.Fatalf("b should be child of a")
	}
	if c.Block.ParentHash() != a.Block.Hash() {
		t.Fatalf("c should be child of a")
	}
	if d.Block.ParentHash() != c.Block.Hash() {
		t.Fatalf("d should be child of c")
	}
	if e.Block.ParentHash() != b.Block.Hash() {
		t.Fatalf("e should be child of b")
	}
	if f.Block.ParentHash() != e.Block.Hash() {
		t.Fatalf("f should be child of e")
	}
	if g.Block.ParentHash() != b.Block.Hash() {
		t.Fatalf("g should be child of b")
	}
	if h.Block.ParentHash() != g.Block.Hash() {
		t.Fatalf("h should be child of g")
	}
	if i.Block.ParentHash() != h.Block.Hash() {
		t.Fatalf("i should be child of h")
	}
