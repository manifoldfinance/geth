...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	chainDb, err := ctx.OpenDatabaseWithOverlayAndFreezer("chaindata", config.DatabaseCache * 3/4, config.DatabaseCache/4, config.DatabaseHandles, config.DatabaseFreezer, config.DatabaseOverlay, "eth/db/chaindata/")
	if err != nil {
		return nil, err
	}
