...
func (ctx *ServiceContext) OpenDatabaseWithOverlayAndFreezer(name string, underlayCache, overlayCache, handles int, freezer, overlayPath, namespace string) (ethdb.Database, error) {
	var chainKv ethdb.KeyValueStore
	var err error
	chainKv, err = rawdb.NewLevelDBDatabase(ctx.ResolvePath(name), underlayCache, handles, namespace)
	...
	root := ctx.ResolvePath("chaindata")
	switch {
	case freezer == "":
	...
	case !filepath.IsAbs(freezer):
		...
		freezer = ctx.ResolvePath(freezer)
	}
