...
func (op *Operation) Apply(db ethdb.Database) ([]byte, error) {
	...
	case OpPut:
		kv := &KeyValue{}
		if err := rlp.DecodeBytes(op.Data, kv); err != nil {
			return nil, err
		}
		...
		if bytes.Equal(kv.Key, []byte("LastBlock")) {
			...
			if err := updateOffset(batch, op); err != nil {
				return nil, err
			}
	...
	case OpAppendAncient:
		a := &AncientData{}
		if err := rlp.DecodeBytes(op.Data, a); err != nil {
			return nil, err
		}
		if err := db.AppendAncient(a.Number, a.Hash, a.Header, a.Body, a.Receipt, a.Td); err != nil {
			return nil, err
		}
	case OpTruncateAncients:
		var n uint64
		if err := rlp.DecodeBytes(op.Data, &n); err != nil {
			return nil, err
		}
		if err := db.TruncateAncients(n); err != nil {
			return nil, err
		}
	case OpSync:
	...
	case OpWrite:
		...
		for _, bop := range operations {
			...
			case OpDelete:
				if err := batch.Delete(bop.Data); err != nil {
					return nil, err
				}
			default:
